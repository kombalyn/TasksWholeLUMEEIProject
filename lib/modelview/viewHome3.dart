...
class ViewHome3 extends GetxController {
  ...
  Future<void> setupWebSocket(String uri, String messageToSend, Function(String) onMessage) async {
    ...
    _channel.stream.listen(
      ...
      onError: (error) {
        if (!completer_3.isCompleted) {
          completer_3.completeError(error);
        }
        _channel.sink.close();
      },
      onDone: () {
  ...
  Future moviesHomeapp() async {
    await setupWebSocket('ws://104.155.99.100:8089', "catitem|49",
            (message) {
            ...
            for (int i = 0; i < parsedList.length; i++) {
              _lastch.add(
                Channel(
                    ...
                    channelImage: parsedList[i][
                    1],
                    //"https://storage.googleapis.com/lomeeibucket/krumplielem.jpg",
                    channelType: "Matek",
                    channelUrl: parsedList[i][1],
                    userAgent: "ali",
                    videoId: parsedList[i][3]),
